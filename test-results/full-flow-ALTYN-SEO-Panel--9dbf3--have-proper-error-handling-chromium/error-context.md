...
- Name: full-flow.spec.ts >> ALTYN SEO Panel - Full E2E Flow >> should have proper error handling
- Location: e2e/full-flow.spec.ts:110:3
...
Expected pattern: /404|login|\/$/
Received string:  "http://localhost:3000/non-existent-page"
```
...
> 117 |     expect(url).toMatch(/404|login|\/$/);
  ...
  146 |     expect(inputCount).toBeGreaterThan(0);
  147 |   });
  148 | 
  149 |   test('should handle navigation', async ({ page }) => {
  150 |     // Navigate to app
  151 |     await page.goto(BASE_URL);
  152 |     await page.waitForLoadState('networkidle');
  153 |     
  154 |     // Try to navigate to login
  155 |     await page.goto(BASE_URL + '/login');
  156 |     await page.waitForLoadState('networkidle');
  157 |     
  158 |     // Should be on login page
  159 |     expect(page.url()).toMatch(/login/);
  160 |     
  161 |     // Try to navigate to register
  162 |     await page.goto(BASE_URL + '/register');
  163 |     await page.waitForLoadState('networkidle');
  164 |     
  165 |     // Should be on register page
  166 |     expect(page.url()).toMatch(/register/);
  167 |   });
  168 | 
  169 |   test('should have proper styling', async ({ page }) => {
  170 |     // Navigate to login
  171 |     await page.goto(BASE_URL + '/login');
  172 |     await page.waitForLoadState('networkidle');
  173 |     
  174 |     // Check if page has styles
  175 |     const bgColor = await page.evaluate(() => {
  176 |       return window.getComputedStyle(document.body).backgroundColor;
  177 |     });
  178 |     
  179 |     // Should have some background color
  180 |     expect(bgColor).toBeTruthy();
  181 |   });
  182 | });
  183 | 
```